...
The trace plots for `unit_precision` and `hosp_precision` do not seem so well.
The other trace plots seem ok. However, the plots for `tech`, `teach` and `beds` are somewhat zigzagging, thus indicating dependency.
None of the plots get stuck.
...
The Gelman-Rubin ANOVA diagnostic (i.e. estimated potential scale reduction factor - PSRF) is close to 1 for all parameters, which is the aim. However, the MCMC error is still very large in comparison with the SD, ranging from 0.5 to 5.0. Additional measures (longer chain, accelerating measure) are necessary.
...
The trace plots for `unit_precision` and `hosp_precision` still do  not seem so well.
The other trace plots seem ok. However, the plots for `tech`, `teach` and `beds` are still somewhat zigzagging, thus indicating dependency.
None of the plots get stuck.
Because of the dependency in the `tech`, `teach` and `beds`, additional measures are required.
<!--- What additional measures could we implement? --->
...
The Gelman-Rubin ANOVA diagnostic (i.e. estimated potential scale reduction factor - PSRF) is close to 1 for all parameters, which is the aim. However, the MCMC error is still fairly very large in comparison with the SD, ranging from 0.3 to 2.5. Additional measures (longer chain, accelerating measure) are necessary.
